<?php 
  ...
  if(mysqli_connect_errno()){ //db연결 실패
    ...
    $strQuery = "SELECT * FROM instructor 
                  order by instructor_idx desc;";
    $queryResult = mysqli_query($connect, $strQuery); // 쿼리 실행
    $numRow = mysqli_num_rows($queryResult); //조회한 행의 개수 
    $instructorData = mysqli_fetch_all($queryResult, MYSQLI_ASSOC);

    if($queryResult && $numRow > 0){ // 쿼리 성공했고 강의 있으면
      $result['status'] = true;
      $result['message'] = "강사조회성공";
      $result['instructor'] = $instructorData;
      // foreach ($userData as $key => $value) {
      //   $result[$key] = $value;
      // }
    }else{ // 쿼리실패 or 강의없음
      $result['status'] = false;
      $result['message'] = "쿼리실패 or 강사없음";
    }
